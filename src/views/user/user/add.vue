	...
	<div class="user-add">
		...
		<el-form size="small" ref="form" :model="form" label-width="180px" :loading="loading">
			...
			<div class="basic-setting-content pl16 pr16">
			  ...
			  <el-form-item label="會員名稱：" :rules="[{ required: true, message: '請填寫會員名' }]" prop="model.nickName">
			    <el-input v-model="form.model.nickName" class="max-w460" ></el-input>
			  </el-form-item>
			  <el-form-item label="Mobile/Tel：(login_id)" :rules="[{ required: true, message: '會員電話' }]" prop="model.mobile">
			     <el-input v-model="form.model.mobile" class="max-w460"></el-input>
			  </el-form-item>
			  <el-form-item label="收貨人名：" :rules="[{ required: true, message: '收貨人名' }]" prop="model.address.name">
				...
				<el-form-item label="收貨人電話：" :rules="[{ required: true, message: '收貨人電話' }]">
				  <el-input v-model="form.model.address.tel" class="max-w460"></el-input>
				</el-form-item>
				<el-form-item label="選地區" prop="model.address.province_id" :rules="[{required: true,message: ''}]">
				  <el-select v-model="form.model.address.province_id" placeholder="省" @change="initCity">
				    <el-option :label="item.name" :value="item.id" v-for="(item,index) in form.region" :key='index'></el-option>
				  </el-select>
				  <span></span>
				  <el-select v-if="form.model.address.province_id!=''" v-model="form.model.address.district_id" placeholder="港九"  @change="initRegion" >
				    <el-option :label="item1.name" :value="item1.id"
				      v-for="(item1,index1) in form.region[form.model.address.province_id]['city']" :key='index1'></el-option>
				  </el-select>
				  <span></span> 
				  <el-select v-if="form.model.address.district_id!=''" v-model="form.model.address.region_id" placeholder="區">
